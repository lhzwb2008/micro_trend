...
策略：Sector-Top + 主线离散度过滤（目前实验中全周期表现最好、最简单的方案）

动机：
"去年大消费、今年AI硬件"本质上是**板块级别**的资金+动量趋势，而不是孤立的个股行为。
个股选股容易被单只股票的特质波动干扰，板块聚合后信号更稳定、也更贴近"识别趋势"的初衷。
但只做"选最强板块"在没有主线的年份（2018/2022/2023）会持续亏损，所以叠加一个"主线是否
存在"的判断：板块之间相对强度的分化程度（离散度）低于近期常态时，说明市场没有清晰主线，
降低仓位；分化明显时，说明确实存在强弱分明的主线，加大仓位去捕捉。

数据（见 get_hk_data.py）：
- 个股全历史日线（新浪接口，覆盖到上市首日，本次样本最早到1998年，回测评估从2018年开始）
- 南向资金个股持仓（东财接口限制，仅约2024-07至今），2024年7月前用中性值填充
- 行业分类（东财"所属行业"）——板块聚类依据

方法：
1. 东财"所属行业"把200只港股通成分股分到28个行业，成员数<4的归并为"其他"
2. 月末把个股特征（相对强度、动量、南向资金变化、成交额趋势）按行业等权聚合成板块特征
3. 板块打分 = 4个特征等权分位数排名，选Top4板块，每个板块内只买个股评分最高的7只
   （平均持仓约21只，而不是板块内全买——精选后收益/夏普/回撤三个指标同时变好，
   见README"参数优化"一节）
4. 【主线离散度过滤 = 择时】：板块间60日相对强度的截面标准差，与最近36个月的滚动中位数
   比较；低于中位数（没有明显分化 = 没有主线）时**清仓**，高于时满仓买入选中的板块。
   这是真正的0/1择时，不是"减半仓位"——测试下来空仓比减半仓位效果更好（详见README）。
   注意：基准必须用"滚动窗口"而不是"从头到今天的全部历史(expanding)"，因为港股通股票池
   在早年成分股很少、离散度天然噪声更大，用expanding会把基准线错误地抬高，导致后期
   被误判为"一直没有主线"（这是本项目实测踩过的一个坑，详见 README）。

参数（均已做过网格搜索，见README"参数优化"）：
- Top4板块 × 板块内精选7只 ≈ 平均持仓21只，主线离散度回看36个月，无主线时清仓。
  均线相关的市场趋势过滤和退守防御板块经测试对整体表现没有稳定的正贡献，本版本不再叠加。
"""
