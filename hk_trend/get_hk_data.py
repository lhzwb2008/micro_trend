...
# -*- coding: utf-8 -*-
"""
港股板块轮动模型 - 数据下载（唯一数据入口脚本）

数据来源：akshare（新浪 + 东方财富接口）
1. 港股通成分股列表 —— 股票池（按成交额取前 N 只）
2. 个股全历史日线（新浪 stock_hk_daily，覆盖到上市首日，远长于东财接口的近2年限制）
   —— 用于计算长周期动量/资金(成交额)趋势特征，使回测能穿越多个牛熊周期
3. 南向资金个股持仓历史（东财，仅约2年）—— 資金趋势的直接特征，2024年7月前用中性值填充
4. 行业分类（东财"所属行业"）—— 板块聚类的依据

（南向资金市场总量长历史曾用于宏观择时overlay实验，实测没有稳定正贡献，最终版脚本
sector_rotation_backtest.py 未使用，这里不再下载）

全部数据增量缓存在 data/cache/，重复运行只补齐缺失部分。
"""
